...
func response(pairs ...[3]string) *geminiResponse {
	r := &geminiResponse{}
	for _, p := range pairs {
		...
		}{From: p[0], To: p[1], Preparation: p[2]})
	}
	return r
}
...
func TestValidateMergesRejectsUnsafeProposals(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			accepted, rejected := validateMerges(response([3]string{tt.from, tt.to}), known(tt.known...))
			if len(accepted) != 0 {
...
func TestValidateMergesAcceptsPluralsAndQualifiers(t *testing.T) {
	raw := response(
		[3]string{"Onions", "onion"},
		[3]string{"garlic cloves", "garlic"},
		[3]string{"medium onion", "onion", "medium"},
	)
	accepted, rejected := validateMerges(raw, known("onions", "onion", "garlic cloves", "garlic", "medium onion"))

	if len(rejected) != 0 {
		t.Fatalf("rejected = %v, want none", rejected)
	}
	if len(accepted) != 3 {
		t.Fatalf("accepted %d merges, want 3", len(accepted))
	}
	// Sorted by source, and lowercased on the way in. The count qualifier
	// retires with the name — the book stores it as the line's unit — so
	// "garlic cloves" carries no preparation; the state qualifier survives.
	if accepted[0].From != "garlic cloves" || accepted[0].To != "garlic" || accepted[0].Preparation != "" {
		t.Errorf("accepted[0] = %+v, want an empty preparation (count qualifiers become units)", accepted[0])
	}
	if accepted[1].From != "medium onion" || accepted[1].To != "onion" || accepted[1].Preparation != "medium" {
		t.Errorf("accepted[1] = %+v, want preparation kept", accepted[1])
	}
	if accepted[2].From != "onions" || accepted[2].To != "onion" {
		t.Errorf("accepted[2] = %+v", accepted[2])
	}
}
