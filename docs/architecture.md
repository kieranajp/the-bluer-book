...
- **Metrics**: Prometheus at `/metrics`. HTTP middleware records request
  ...
  `backend.md`.
- **Dashboard**: a ready-to-import Grafana dashboard lives at `monitoring/dashboard.json`
  (overview, HTTP, recipes, pantry, chat, database, Go runtime). Pick a Prometheus
