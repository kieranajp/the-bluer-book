...
func (h *RecipeMCPHandler) RemoveFromMealPlan(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	if err != nil {
		h.logger.Error().Err(err).Str("recipe_id", recipeIDStr).Msg("Failed to get recipe before removing from meal plan via MCP")
	...
	err = h.recipeService.RemoveFromMealPlan(ctx, recipeID)
	if err != nil {
		h.logger.Error().Err(err).Str("recipe_id", recipeIDStr).Msg("Failed to remove recipe from meal plan via MCP")
		return nil, fmt.Errorf("failed to remove recipe from meal plan: %w", err)
	}
