...
//
// Skipped unless BLUER_BOOK_TEST_DSN points at a migrated, disposable database.
// CI has no Postgres service, so these do not run there:
//
//	BLUER_BOOK_TEST_DSN='postgres://postgres:postgres@127.0.0.1:55432/bluer?sslmode=disable' go test ./internal/infrastructure/storage/repository/
func testDB(t *testing.T) (*sql.DB, *db.Queries) {
	t.Helper()
	dsn := os.Getenv("BLUER_BOOK_TEST_DSN")
	if dsn == "" {
		t.Skip("BLUER_BOOK_TEST_DSN not set; skipping database integration tests")
	}
...
func newRepos(t *testing.T) (RecipeRepository, PantryRepository, *sql.DB) {
	...
	return NewRecipeRepository(sqlDB, log), NewPantryRepository(sqlDB, log), sqlDB
}

// testCtx acts inside a fresh home, cleaned up with the test. Every tenant
// operation in this file goes through it — the whole suite is home-scoped.
func testCtx(t *testing.T, sqlDB *sql.DB) (context.Context, uuid.UUID) {
	t.Helper()
	home := makeHome(t, sqlDB, "integration test home")
...
// saveRecipe saves inside ctx's home. The context is handed back so the same
// scope reads the recipe again.
func saveRecipe(t *testing.T, repo RecipeRepository, ctx context.Context, name string, lines ...recipe.RecipeIngredient) *recipe.Recipe {
	t.Helper()
	saved, err := repo.SaveRecipe(ctx, recipe.Recipe{
		Name:        name,
		Ingredients: lines,
...
func TestSaveRecipeReusesIngredientsAcrossCasing(t *testing.T) {
	recipes, _, sqlDB := newRepos(t)
	ctx, _ := testCtx(t, sqlDB)

	saveRecipe(t, recipes, ctx, "First", ingredientLine("Olive Oil", "tbsp", "", 2))
	saveRecipe(t, recipes, ctx, "Second", ingredientLine("olive oil", "tbsp", "", 1))
	saveRecipe(t, recipes, ctx, "Third", ingredientLine("  OLIVE OIL  ", "tbsp", "", 1))
...
func TestSaveRecipeKeepsOneIngredientInTwoComponents(t *testing.T) {
	...
	saved := saveRecipe(t, recipes, ctx, "Katsu",
...
func TestSaveRecipeCollapsesRepeatsWithinOneComponent(t *testing.T) {
	...
	saved := saveRecipe(t, recipes, ctx, "Doubled",
...
func TestUpdateRecipeCollectsOrphanedIngredients(t *testing.T) {
	...
	saved := saveRecipe(t, recipes, ctx, "Typo", ingredientLine("corriander", "g", "", 10))
...
func TestPantryResolvesNamesAndReportsUnknownOnes(t *testing.T) {
	...
	saveRecipe(t, recipes, ctx, "Roast", ingredientLine("Olive Oil", "tbsp", "", 2))
...
func TestPantryResolvesThroughAliases(t *testing.T) {
	...
	saveRecipe(t, recipes, ctx, "Salsa", ingredientLine("fresh coriander", "g", "", 20))
	home, _ := auth.HomeID(ctx)
	if err := inTestHome(sqlDB, home, func(tx *sql.Tx) error {
		_, err := tx.Exec(`
			INSERT INTO ingredient_aliases (alias, ingredient_id)
			SELECT 'cilantro', uuid FROM ingredients WHERE canonical_name = 'fresh coriander'`)
		return err
	}); err != nil {
...
func TestShoppingListSkipsStaplesAndStockedItems(t *testing.T) {
	...
	saved := saveRecipe(t, recipes, ctx, "Dinner",
